...
def start(difLevel):
    try:
        ...
        while True:
            ...
            #Метод\код вычисляет для каждой ячейки набор соседних ячеек(групп), количество открытых и неоткрытых ячеек
            #Пример одного элемента:
            #y-координата соседней ячейки,x-координата соседней ячейки, Состояние соседней ячейки(открыто\закрыто), y,x индексы кому приходятся соседом
            
            findNeighboringCells(field)
